...
################################################################
model =CRNO2d(modes,modes,width,101,101).cuda()
...
for ep in range(epochs):
    ...
    with open(f"CRNO4/CRNO_{random_seed}.txt", "a") as f:
        if ep % 20 == 0 or ep == epochs - 1:
            log_message = (f"{ep}, {t2-t1}, {train_l2}, {test_l2}\n")
            f.write(log_message)  
            
torch.save(model.state_dict(), f"CRNO4/CRNO_{random_seed}.pth")    