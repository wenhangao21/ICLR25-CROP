...
class FNO2d(nn.Module):
    def __init__(self, modes1, modes2, width, in_size, latent_size):
        ...
        self.p = nn.Conv2d(10, self.width, 1) # input channel is 12: the solution of the previous 10 timesteps + 2 locations (u(t-10, x, y), ..., u(t-1, x, y),  x, y)
...
for sub in [1,2,4, 8]:
    ...
    for seed in range(10):
        ...
        model = FNO2d(modes, modes, width, S, 32).cuda()
        model.load_state_dict(torch.load(f'CROP_{seed}.pth'))
        ...
        with torch.no_grad():
            for xx, yy in test_loader:
                ...
                for t in range(0, T, step):
                    ...
                    test_l2_each_time[t] += myloss(im.reshape(batch_size, -1), y.reshape(batch_size, -1)).item()
                test_l2_step += loss.item()
                test_l2_full += myloss(pred.reshape(batch_size, -1), yy.reshape(batch_size, -1)).item()
        rel_l2_values.append(test_l2_step/ntest/T)
        #print("one step test l2: ",test_l2_step / ntest / (T / step), "full l2: ", test_l2_full / ntest)
    ...
    test_l2_each_time /= (ntest*10)

    print(f"Mean L2 = {mean_rel_l2}, Std L2 = {std_rel_l2}")
    print(test_l2_each_time)
    print(sum(test_l2_each_time))
